...
<!--One_Wrap-->
 ...
 <?php 
  $myid = intval($_SESSION["UID"]);
 	$novas = mysql_query("SELECT * FROM `message` where `to` = ".$_SESSION["UID"]." ORDER BY `ID` DESC LIMIT 0, 50 ");
		while($result = mysql_fetch_array($novas)){
    ...
    $com02 = mysql_query("SELECT `username` FROM `admin` WHERE `ID` = $result[from] ");
     ...
     <tr>
        ...
        <td align="left"><a href="view-message.php?tag=<?php echo $result["ID"]; ?>" target="_self"><span class="<?php if($result["read"]=="NO"){?>green_highlight<?php }else{?>grey_highlight<?php } ?> pj_cat">
          <?php
        ...
        <td align="center"><strong><?php echo $result["time"]; ?></strong></td>
        <td align="left"><a href="#" style="font-family:Tahoma;"><?php echo $res; ?></a></td>
        <td align="center" valign="middle">
        	<span class="data_actions iconsweet"><a class="tip_north" original-title="Delete This Question" href="delete-email.php?SESSION=ECCP&=SESSIONID=<?php echo rand(11111, 99999); ?>&T=<?php echo $result["ID"]; ?>">X</a></span></td>
      </tr>                   
	<?php } ?>
  </table>
