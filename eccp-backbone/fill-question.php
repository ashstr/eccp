...
<?php if($_REQUEST["action"]=="update"){
		require("fill-question-action.php");
		}else{  ?>
...
<!--One_Wrap-->
 ...
 $catId = intval($cleanHTML03);
 	...
 	$Comm1 = mysql_query("SELECT * FROM `questions` WHERE `pending` != 0 AND `cat` = '$catId' ORDER BY `ID` DESC LIMIT 0, 50 ");
     ...
     <tr>
        ...
        <td align="left"><?php
			...
			if(empty($res)){ echo "<em><strong>Undefined</strong></em>"; }else{ echo htmlentities($res);}
			  ...
			  ?></td>
        <td align="left"><a href="view-questions.php?SESSION=ECCP&SESSIONID=<?php echo rand(11111, 99999); ?>&SESSIONID=<?php echo rand(11111111, 999999999);  echo "&MCV=".generateCodeBig(40); ?>&qid=<?php echo $result["ID"]; ?>&QUE=2&ex=<?php echo intval($result["ID"]); ?>&PartID=<?php echo rand(11111111, 999999999); echo generateCodeBig(40); ?>" target="_blank">
		<?php
	  ...
	  echo  html_entity_decode($valv);
        ...
        <td align="center">
          <span class="data_actions iconsweet">
        	<a class="tip_north" original-title="Add This Question" href="fill-question-action.php?SESSION=ECCP&SESSIONID=<?php echo rand(11111, 99999); ?>&SESSIONID=<?php echo rand(11111111, 999999999);  echo "&MCV=".generateCodeBig(40); ?>&q=<?php echo $result["ID"]; ?>&ExamID=<?php echo intval($_GET["ExamID"]); ?>&PartID=<?php echo rand(11111111, 999999999); echo generateCodeBig(40); ?>" target="_blank">L</a>
         </span></td>
      </tr>                   
	<?php } ?>
  </table>
