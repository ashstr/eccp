...
<!--One_Wrap-->
 ...
 <?php 
	...
	$cleanHTML03 = cleanIt($_GET['point'],'NO_HTML');	
	$ExamID = intval($cleanHTML03); 
	
 	$Comm1 = mysql_query("SELECT * FROM `container` WHERE  `eid` = '$ExamID'  ");
		while( $result = mysql_fetch_array($Comm1)){ ?>
     <tr>
        <td align="center"><?php echo $result["ID"]; ?></td>
        
        <!--<td align="left"><?php
			//$com02 = mysql_query("SELECT * FROM `subject` WHERE `ID` = $result[cat] ");
			// $com03 = mysql_fetch_array($com02);
			// $res =  strtoupper($com03["title"]);
			// if(empty($res)){ echo "<em><strong>Undefined</strong></em>"; }else{ echo $res;}
			?></td>-->
            
        <td align="left">
		<?php
			$com02     = mysql_query("SELECT * FROM `questions` WHERE `ID` = $result[qid] ");
			 $com03    = mysql_fetch_array($com02);
			 $valuesMe = html_entity_decode($com03["ques"]);
			 $res      = cleanIt($valuesMe,'NO_HTML');
			 if(empty($res)){ echo "<em><strong>Undefined</strong></em>"; }else{ echo str_replace("<p>",' ',$res); }
			?>
			<?php
	  ...
	  echo  html_entity_decode($valv);
        ...
        <td align="center"><span class="data_actions iconsweet"><a class="tip_north" original-title="View Full Question" href="details-question.php?SESSION=ECCP&=SESSIONID=<?php echo rand(11111, 99999); ?>&T=<?php echo $result["ID"]; ?>" target="_blank">}</a> <a class="tip_north" original-title="Edit" href="edit-question.php?SESSION=ECCP&=SESSIONID=<?php echo rand(11111, 99999); ?>&T=<?php echo $result["ID"]; ?>">l</a><a class="tip_north" original-title="Delete This Question" href="delete-question.php?SESSION=ECCP&=SESSIONID=<?php echo rand(11111, 99999); ?>&T=<?php echo $result["ID"]; ?>">X</a></span></td>
      </tr>                   
	<?php } ?>
  </table>
