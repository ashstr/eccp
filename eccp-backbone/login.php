...
<?php if($_REQUEST["action"]=="login"){ 
 ...
 if(!empty($_POST["user_name"])){
   $sa=mysql_query("select * from `admin` where `username`='$_POST[user_name]' and `password`='$_POST[password]'") or die(mysql_error());
     if(mysql_num_rows($sa)>0){
	
	   $ro=mysql_fetch_array($sa);
			$_SESSION["user_admin"] 	  = $ro["ID"];
			$_SESSION["user_admin_name"] = $ro["username"];
